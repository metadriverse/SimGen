...
class SimGenPipeline(Pipeline):
    ...
    def mix_cond(self, back_cond, fore_cond):
        ...
        mask[..., 0] = np.isin(fore_cond[..., 0], [0]).astype(np.uint8) & np.isin(fore_cond[..., 1], [0]).astype(
            np.uint8) & np.isin(fore_cond[..., 2], [0]).astype(np.uint8)
        back_cond = np.where(mask == 0, 0, back_cond)
        return np.maximum(fore_cond, back_cond).astype(np.float32) / 255.0

    def _process(self, sn_depth_image, sn_seg_image, prompt,
                 num_samples, image_resolution_h, image_resolution_w, ddim_steps, strength, scale, seed, eta):
        ...
        with torch.no_grad():
            ...
            detected_maps_list = [sn_depth_image,
                                  sn_seg_image,
                                  ]
            self.split_back_ground = False
            ...
            def pre_process(condition):
                ...
                condition = cv2.resize(condition, dsize=(image_resolution_w, image_resolution_h), fx=1, fy=1,
