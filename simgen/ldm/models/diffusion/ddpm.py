    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        if self.make_it_fit:
            ...
            for name, param in tqdm(itertools.chain(self.named_parameters(), self.named_buffers()),
                                    desc="Fitting old weights to new weights", total=n_params):
                if not name in sd:
                    continue
        ...
        missing, unexpected = self.load_state_dict(
            sd, strict=False
        ) if not only_model else self.model.load_state_dict(
            sd, strict=False
        )
...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        # print(self.model.conditioning_key, self.force_null_conditioning, cond_key, self.cond_stage_key, self.first_stage_key, force_c_encode, self.cond_stage_trainable)
    ...
    def progressive_denoising(
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {
                    ...
                    if not isinstance(cond[key], list) else list(map(lambda x: x[:batch_size], cond[key]))
                    for key in cond
                }
            else:
        ...
        iterator = tqdm(
            reversed(range(0, timesteps)), desc='Progressive Generation', total=timesteps
...
class LatentFinetuneDiffusion(LatentDiffusion):
    ...
    def __init__(
        ...
        concat_keys: tuple,
        finetune_keys=(
            "model.diffusion_model.input_blocks.0.0.weight", "model_ema.diffusion_modelinput_blocks00weight"
        ),
