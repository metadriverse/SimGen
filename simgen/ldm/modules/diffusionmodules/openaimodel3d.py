...
# Temporal relations following GenAD-private-genad_improved_yjz/sgm/modules/openaimodel3d.py
class TemporalRelation(nn.Module):
    def __init__(self, model_dim, kernel_size=3, dropout=0.):
        super().__init__()
        padding = kernel_size // 2
        self.temporal_reasoning = nn.Sequential(
            nn.GroupNorm(32, model_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            zero_module(conv_nd(3, model_dim, model_dim, kernel_size=(kernel_size, 1, 1), padding=(padding, 0, 0))),
        )

    def forward(self, x):
        return x + self.temporal_reasoning(x)


class SingleTemporalRelation(nn.Module):
    def __init__(self, model_dim, dropout=0., num_frames=1):
        super().__init__()
        self.num_frames = num_frames
        self.temporal_module = TemporalRelation(model_dim, dropout=dropout)

    def forward(self, x):
        x = rearrange(x, "(b t) c h w -> b c t h w", t=self.num_frames)
        x = self.temporal_module(x)
        x = rearrange(x, "b c t h w -> (b t) c h w")
        return x


class DoubleTemporalRelation(nn.Module):
    def __init__(self, model_dim, dropout=0., num_frames=1, checkpoint=True):
        super().__init__()
        self.num_frames = num_frames
        self.temporal_module1 = TemporalRelation(model_dim, dropout=dropout)
        self.temporal_module2 = TemporalRelation(model_dim, dropout=dropout)
        self.checkpoint = checkpoint

    def _forward(self, x):
        x = rearrange(x, "(b t) c h w -> b c t h w", t=self.num_frames)
        x = self.temporal_module2(self.temporal_module1(x))
        x = rearrange(x, "b c t h w -> (b t) c h w")
        return x
...
# modified from GenAD-private-genad_improved_yjz/sgm/modules/diffusionmodules/openaimodel3d.py
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    """
    A sequential module that passes timestep embeddings to the children that
    support it as an extra input.
    """
    def forward(self, x, emb, context=None, cond_mask=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTemporalTransformer):
                x = layer(x, context, cond_mask)
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if num_frames > 1:
            self.temporal_layer = DoubleTemporalRelation(
                model_dim=self.out_channels, dropout=dropout, num_frames=num_frames
            )
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        :param cond_mask: condition mask indicating the condition frames.
        ...
        if self.num_frames > 1 and context.shape[0] != x.shape[0]:
            context = repeat_as_img_seq(context, self.num_frames)
            assert context.shape[0] == x.shape[0]

        if self.num_classes is not None:
            if self.num_frames > 1 and y.shape[0] != x.shape[0]:
                y = repeat_as_img_seq(y, self.num_frames)
                assert y.shape[0] == x.shape[0]
            emb = emb + self.label_emb(y)

        h = x.type(self.dtype)
        # cond_mask = torch.ones(x.shape[0], requires_grad=False).to(x.device).float()
        # cond_mask = cond_mask[..., None].float()
        for module in self.input_blocks:
            # h = module(h, emb, context, cond_mask)
            h = module(h, emb, context)
            hs.append(h)
        # h = self.middle_block(h, emb, context, cond_mask)
