...
def init_local(sd_weights_path, config_path, output_path):
    ...
    for sk in scratch_dict.keys():
        if sk.replace('local_adapter3d.', 'local_adapter.') in pretrained_weights.keys():
            target_dict[sk] = pretrained_weights[sk.replace('local_adapter3d.', 'local_adapter.')].clone()
        else:
